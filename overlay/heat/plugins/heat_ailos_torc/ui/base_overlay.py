...
from overlay.heat.plugins.heat_ailos_torc.ocr.range_ocr import TESSERACT_OK, ocr_capture_range
from overlay.heat.plugins.heat_ailos_torc.ocr.ocr_ui import OCRUiMixin
...
class BaseSACLOSOverlay(OCRUiMixin, RangefinderUiMixin, HudUiMixin):
    def __init__(self, root, image_path=None, tracking_image_path=None, margin_x=200, margin_y=200,
                 config_filename="heat_ailos_torc.json", fuse_config=None):
        self.root = root
        self.root.title("SACLOS Overlay")
        self._fuse_config = fuse_config
        self.fuse_mode = fuse_config is not None
        ...
        # In FUSE mode the host owns Ctrl+L and Ctrl+P — suppress Tk bindings.
        if self.fuse_mode:
            for widget in [self.root, self.canvas]:
                widget.bind("<Control-l>", lambda e: None)
                widget.bind("<Control-L>", lambda e: None)
                widget.bind("<Control-p>", lambda e: None)
                widget.bind("<Control-P>", lambda e: None)
    ...
    def _save_config(self):
        ...
        # Subclasses inject additional keys via _save_hud_config / _add_extra_config.
        self._save_hud_config(config_dict)
        self._add_extra_config(config_dict)
        if self._fuse_config is not None:
            self._fuse_config.update(config_dict)
        else:
    ...
    def _load_config(self):
        if self._fuse_config is not None:
            config = self._fuse_config   # already loaded by plugin before construction
        else:
    ...
    def _fuse_teardown(self):
        """Called by the FUSE plugin's teardown(). Cleans up without destroying the host root."""
        self._capture_hud_positions()
        self._save_config()
        if self.update_timer_id is not None:
            self.root.after_cancel(self.update_timer_id)
    ...
    def _on_calibrate(self):
        """FUSE lifecycle hook — mirror of the existing unlock path."""
        self._exit_to_calibrate()

    def _on_locked(self):
        """FUSE lifecycle hook — go directly to locked (skips adjust_bounds)."""
        if self.state in ("calibrate", "adjust_bounds"):
            # Ensure origin is set when adjust_bounds phase was never entered.
