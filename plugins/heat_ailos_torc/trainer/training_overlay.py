...
from heat_ailos_torc.trainer.training_ql import TrainingQuickLabelMixin
...
class TrainingOverlay(TrainingQuickLabelMixin, QuickLabelHudMixin, BaseSACLOSOverlay):
    ...
    def _tql_show_placeholder_graphs(self):
        ...
        if self._ql_sim_canvas:
            from heat_ailos_torc.predictor.ql_hud import _traj_to_cumulative
            sample_traj = [
                {'t': 0.0, 'dx': 0, 'dy': 0},
                {'t': 0.05, 'dx': 3, 'dy': -1},
                {'t': 0.10, 'dx': 5, 'dy': -2},
    ...
    def _open_refiner(self, capture):
        ...
        from heat_ailos_torc.refiner.trajectory_editor import TrajectoryEditorWindow
