...
export interface FlameWrapOptions {
  ...
  spread?: number;
  /**
   * Layout reservation for the output canvas, in CSS pixels. Defaults to
   * `height`/`spread`. Set these to the largest value you will ever animate
   * `height`/`spread` to: the canvas box is then sized once and stays put, and
   * the flame simply draws smaller inside it. Without that, animating `height`
   * resizes the canvas every frame, and resizing clears the drawing buffer —
  ...
  scorch?: number;
  /**
   * Burn the content's silhouette instead of its bounding box. The burning edge
   * is derived from the content alpha (mip-blurred into a pseudo-SDF), so
   * non-rectangular art — circles, triangles, icons — is traced instead of a
   * rounded rect. Needs html-in-canvas; falls back to the box without it.
   */
  traceContent?: boolean;
  /**
   * Treat the slot as a shape stand-in only: it sizes the burning region (and
   * with `traceContent`, the traced silhouette) but is never drawn. Use when the
   * real artwork renders outside the effect — otherwise the stand-in is painted
   * underneath it and reads as content. Works with or without `traceContent`.
   */
  traceOnly?: boolean;
}
...
// reserve* are a layout concern for the Vue wrapper, never a shader uniform.
const DEFAULTS: Required<Omit<FlameWrapOptions, "reserveHeight" | "reserveSpread">> = {
  color: [0.31, 0.54, 1],
  intensity: 0.5,
  height: 170,
  spread: 8,
...
/* Mip-blurred content alpha: ~1 deep inside the silhouette, decaying outside.
   Levels stay mid-pyramid — the coarsest levels of a small texture are a global
   average, which flattens the field into noise. The caller is expected to leave
   transparent padding around the art so CLAMP_TO_EDGE has nothing to smear. */
float contentField (vec2 cuv) {
  vec2 uv = vec2(clamp(cuv.x, 0.0, 1.0), 1.0 - clamp(cuv.y, 0.0, 1.0));
  return 0.40 * textureLod(uContent, uv, 1.5).a
    + 0.35 * textureLod(uContent, uv, 2.5).a
    + 0.25 * textureLod(uContent, uv, 3.5).a;
}
...
void main () {
  ...
  /* Silhouette mode: swap the box SDF for the content field, and take the
     up-facing weight from the field's gradient so tall flames rise off whatever
     part of the shape actually points up (a triangle's slopes, a circle's cap)
     while sides and undersides keep the tighter glow. */
  if (uTrace > 0.5) {
    vec2 cuvT = (rel + uRectHalf) / (2.0 * uRectHalf);
    vec2 rectSize = 2.0 * uRectHalf;
    /* Central differences over a few texels, not dFdx/dFdy: screen-space
       derivatives are constant across each 2x2 quad, which quantises the
       distance into visible blocks. */
    vec2 texel = 1.0 / vec2(textureSize(uContent, 0));
    vec2 e = texel * 3.0;
    float fld = contentField(cuvT);
    float fx = contentField(cuvT + vec2(e.x, 0.0)) - contentField(cuvT - vec2(e.x, 0.0));
    float fy = contentField(cuvT + vec2(0.0, e.y)) - contentField(cuvT - vec2(0.0, e.y));
    vec2 grad = vec2(fx / (2.0 * e.x * rectSize.x), fy / (2.0 * e.y * rectSize.y));
    float gmag = length(grad);
    /* First-order distance estimate: (threshold - field) / |grad| lands in
       pixels, so the field's own falloff sets the scale — and it extrapolates
       linearly, so flames still reach past the blur radius. */
    float dC = clamp((0.42 - fld) / max(gmag, 1e-4), -2.0 * unit, 2.0 * unit);
    if (gmag < 1e-4) dC = 2.0 * unit;
    float upness = -grad.y / max(gmag, 1e-5);
    float wTopC = S(0.1, 0.8, upness);
    front = dC + mix(biteSB, biteTop, wTopC);
    wTop = wTopC;
  }
  ...
  /* traceOnly: the slot shaped the fire above and is not drawn, so fall through
     to the fire-only composite. */
  if (uHasContent < 0.5 || uTraceOnly > 0.5) {
...
export function createFlameWrap(
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        contentDirty = true;
        // Upload and clear inside the paint pass. Deferring to the next frame
        // leaves the drawn bitmap on screen until then, which flashes the slot
        // content over the page — visible as flicker on every repaint.
        uploadContent();
        wake();
      } catch {}
    };
  ...
  // traceContent samples coarse mip levels, which needs a mipmap-complete
  // texture; plain box mode keeps the cheaper non-mipmapped filter.
  const tracing = (): boolean => Boolean(config.traceContent) && htmlInCanvas;
  function syncMipFilter() {
    gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
    gl!.texParameteri(
      gl!.TEXTURE_2D,
      gl!.TEXTURE_MIN_FILTER,
      tracing() ? gl!.LINEAR_MIPMAP_LINEAR : gl!.LINEAR,
    );
    if (tracing()) gl!.generateMipmap(gl!.TEXTURE_2D);
  }
  syncMipFilter();
  ...
  function render() {
    ...
    gl!.uniform1f(uniforms.uTrace, tracing() ? 1 : 0);
    gl!.uniform1f(uniforms.uTraceOnly, config.traceOnly ? 1 : 0);
  ...
  return {
    setOptions(next) {
      ...
      if (tracing() !== wasTracing) {
        syncMipFilter();
        // The source canvas is cleared after each upload, so the texture can
        // only be rebuilt (with mips) from a fresh paint.
...
const outputStyle = computed(() => {
  // Sized off the reservation, not the live value, so animating height/spread
  // never resizes (and therefore never clears) the output canvas.
  const reach = Math.round(Math.max(props.reserveHeight ?? props.height ?? 170, 24) * 1.5) + 40;
  const glow = Math.round(Math.max(props.reserveSpread ?? props.spread ?? 8, 8) * 3) + 16;
  return {
